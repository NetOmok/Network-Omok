...
public class OmokClient extends JFrame implements Runnable, ActionListener {
   ...
   */
      ...
      try{
         while((msg=reader.readLine())!=null){
            ...
            /*else if(msg.startsWith("[ROOM]")){    // 방에 입장
               ...
               if(!msg.equals("[ROOMNAME]0")){          // 대기실이 아닌 방이면
                  //enterButton.setEnabled(false);
                  //exitButton.setEnabled(true);
                  rList.add(msg.substring(10));
                  infoView.setText(msg.substring(10) + "방에 입장하셨습니다.");
               }
               else 
                  infoView.setText("대기실에 입장하셨습니다.");
               //roomNumber=Integer.parseInt(msg.substring(6));     // 방 번호 지정
            ...
            else if(msg.startsWith("[BLOWROOM]")) { //동준아, player수를 count해서 0이면 방 폭파
               int count = pList.getItemCount();
               if(count == 0) 
                  rList.remove(msg.substring(10));
               else 
                  continue;
            }
   ...
   /*private void roomsInfo() {
      ...
      if(!roomNameBox.equals(""))
         pInfo.setText("총 대기실 : " + count + "개");
      else 
   }*/
   private int playersCount(String roomname) {
      int count = pList.getItemCount();
